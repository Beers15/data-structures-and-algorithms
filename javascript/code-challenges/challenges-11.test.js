...
const count = (target, input) => {
  ...
  input.forEach(subArr => {
    let targetCount = subArr.filter((num) => {
      if(num !== target) {
        return false;
      }
      return true;
    });
    filteredInput.push(targetCount);
  });
  return filteredInput.reduce((targetCount,subArr) => {
    targetCount = targetCount + subArr.length;
    return targetCount;
  }, 0);
};
/* ------------------------------------------------------------------------------------------------
...
const divisibleByFiveTwoToThePower = input => input.map(subArr => subArr.filter(num => typeof num === 'number' && num % 5 === 0 ? true : false).map(num => Math.pow(2, num)));
...
let findMaleAndFemale = data => data.filter(character => character.gender === 'female' || character.gender === 'male').map(character => character.name).join(' and ');
