...
const totalCharacters = arr => {
  let sum = 0;
  sum += Object.keys(arr).length;

  Object.entries(arr).forEach(entry => {
    if(entry[1].children) {
      sum += entry[1].children.length;
    }
    if(entry[1].spouse) {
      ++sum;
    }
...
const houseSize = arr => {
  ...
  Object.entries(arr).forEach(entry => {
    ...
    let existingHouse = sizes.filter(house => {
      if(house.name === entry[1].house) {
        return house;
      }
    });
    if(existingHouse.length > 0) {
      existingHouse[0] = {house: entry[1].house, members: existingHouse[0].members + sum};
    } else {
      sizes.push({house: entry[1].house, members: sum});
    }
  });
...
const houseSurvivors = arr => {
  ...
  Object.entries(arr).forEach(entry => {
    ...
    if(entry[1].spouse) {
      if(entry[1].spouse in deceasedSpouses) {
        ++sum;
      }
    }


    let existingHouse = houseSurvivors.filter(house => {
      if(house.name === entry[1].house) {
        return house;
      }
    });
    if(existingHouse.length > 0) {
      ...
      houseSurvivors.push({house: entry[1].house, members: sum});
    }
  });

  return houseSurvivors;
};
