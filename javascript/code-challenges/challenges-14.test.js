...
const screenForNames = arr => arr.filter(word => /^(Mr|Dr|Mrs|Ms).( )[a-zA-z]/g.test(word));
...
let biggerThanLuke = arr => arr.filter(char => char.height > 172).map(char => char.name).join(' - ');
...
const helpCheck = (row1, col1, row2, col2, row3, col3) => {
  ...
  else if(col1[0] === col2[0] && col2[0] === col3[0]) {
    return true;
  }
  else if(col1[1] === col2[1] && col2[1]=== col3[1]) {
    return true;
  }
  else if(col1[2] === col2[2] && col2[1] === col3[2]) {
    return true;
  }
  return false;
};
