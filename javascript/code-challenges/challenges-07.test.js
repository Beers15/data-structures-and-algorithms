...
const removeThree = (idx, arr) => [...arr.slice(0, idx), ...arr.slice(idx+3, arr.length)];
...
const listFoods = recipe => {
  ...
  for(let ingredient of recipe.ingredients) {
    let str = ingredient.slice(ingredient.indexOf(' '), ingredient.length).trim();
    str = str.slice(str.indexOf(' '), str.length).trim();
    result.push(str);
  }
...
const splitFoods = recipe => {
  ...
  recipe.ingredients.forEach(ingredient => {
    ...
    result.push(arr.splice(2, arr.length).join(' '));
  });
  return result;
};
...
const extractVowels = str => {
  ...
  for(let char of str) {
    if(!(vowels.includes(char.toLowerCase()))) {
      noVowelStr = noVowelStr + char;
    } else {
      vowelStr = vowelStr + char;
    }
  }
  vowelStr = vowelStr.split('').sort((a, b) => a < b ? -1 : 1).join('');
