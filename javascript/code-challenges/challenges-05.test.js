...
const calculateAverage = arr => {
  const accumulator = arr.reduce((accumulator, num) => {
    ++accumulator.count;
    accumulator.sum += num;
...
const extractChildren = arr => {
  const peopleWithA = arr.filter(person => {
    if(person.name.includes('a')) {
      return person;
    }
  });

  const children = peopleWithA.reduce((children, parent) => {
    if(parent.children) {
