...
const validateWord = word => /^[a-z A-Z]{5,10}$/.test(word);
...
const hasNumber = string => /[a-z A-Z]{1,}\d{1,}/.test(string);
...
const validatePhoneNumber = phoneNumber => {
  return /^(\(\d{3}\)|\d{3}) *-{0,1}\d{3} *-{0,1}\d{4}$/.test(phoneNumber);
};
