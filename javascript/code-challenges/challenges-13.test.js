...
const standardizePhoneNumbers = arr => {
  return arr.map(str => {
    return str.split('').filter(char => (char !== ')' && char !== '(' && char !== ' ' && char !== '-')).join('');
  });
};
...
const onlyOddChars = str => str.split('').filter((char, idx) => idx % 2 !== 0).join('');
...
const unenrollBrook = arr => arr.map(roster => roster.filter(person => !person.includes('Brook')));
